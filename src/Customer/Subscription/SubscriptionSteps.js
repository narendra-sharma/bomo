        ...
        {step === 0 && <>
          ...
          <div className="p-4 px-2 px-md-5">
            <div className="subscription-data mb-3 row no-gutters align-items-center w-secUpTo0">
              ...
              <span className={`increament  ${(pieces > firstUpTo) && 'cursor-pointer'}`} onClick={() => (pieces > 1) && decrease()}>-</span>
              <span className="subscription-count">{pieces}</span>
              <span className="increament cursor-pointer g-0" onClick={() => increase()}> +</span>
              </div>
              <div className="col-md-4">
                {!user?.plan_id && <>
                  <div className="savings saving-bg-color rounded py-1 g-0 mb-1">Order up to {pieces} items in one bulk request, or split them as needed</div>
                  <div className="savings rounded py-1 g-0 mb-2">Unlimited Revisions. {(pieces/firstUpTo).toFixed(0)} at a time with your current Plan</div>
                </>}
